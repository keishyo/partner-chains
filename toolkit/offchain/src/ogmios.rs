...
#[allow(dead_code)]
pub async fn query_tip(client: &HttpClient) -> Result<TipResponse, anyhow::Error> {
	client
		.request("queryLedgerState/tip", ObjectParams::new())
		.await
		.map_err(|e| anyhow!("Couldn't get tip, because of {}", e.to_string()))
}
